...
#HOST_KEY = paramiko.RSAKey(filename='server.key', password='qwerty1234')
...
def detect_url(command, IP):
	...
	if result:
		for ar in result:
			for url in ar:
				if url != '':
					logging.info('New URL detected ({}): '.format(IP, url))
	...
	if ip_result:
		for ip_url in ip_result:
			if ip_url != '':
				logging.info('New IP-based URL detected ({}): '.format(IP, ip_url))
